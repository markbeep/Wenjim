...
async function loadHistory(
  ...
  if (activities.length === 0 || locations.length === 0 || !from || !to)
    return [];
  let orderByKey = "date";
  switch (orderBy) {
...
export function useHistory(
  ...
  const { isError, isLoading, data } = useQuery(
    ["history", activities, locations, from, to, orderBy, desc],
...
async function loadHistoryLine(
  activities: string[],
  locations: string[],
  from: Date,
  to: Date,
...
export function useHistoryLine(
  activities: string[],
  locations: string[],
  from: Date,
  to: Date,
  ...
  const { isError, isLoading, data } = useQuery(
    ["historyline", activities, locations, from, to],
...
export function useWeekly(
  ...
  const { isError, isLoading, data } = useQuery(
    ["weekly", activities, locations, from, to],
